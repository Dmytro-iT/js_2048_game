...
function render() {
    ...
    if (st === "start") startMessage.classList.remove("hidden");
    else if (st === "win") winMessage.classList.remove("hidden");
    else if (st === "lose") gameOverMessage.classList.remove("hidden");
    if (st !== "start") {
...
class Game {
    ...
    _slideAndMergeLine(line) {
        ...
        for(let i = 0; i < this.size - 1; i++)if (currentLine[i] !== 0 && currentLine[i] === currentLine[i + 1]) {
            currentLine[i] *= 2;
    ...
    _rotate(board) {
        ...
        for(let r = 0; r < this.size; r++)for(let c = 0; c < this.size; c++)newBoard[r][c] = board[this.size - 1 - c][r];
        return newBoard;
    }
    _performMove(transform) {
        if (this.status !== "playing") return false;
        let transformedBoard = this.board;
        if (transform) transformedBoard = transform(this.board);
        let moved = false;
